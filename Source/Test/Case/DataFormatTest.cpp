...
namespace XidiTest
{
  ...
  /// Given the information needed to construct a data format object and the data format
  /// specification that is expected to result, constructs the data format object, ensures success,
  /// and ensures expectation matches actual result. Once the object has been created, iterates
  /// through the specification and verifies that the data format object correctly translates
  /// between elements and offsets. Causes the test case to fail if any of the checks or operations
  /// are unsuccessful.
  /// @param [in] appFormatSpec Test data in the form of an application data format specification.
  /// @param [in] controllerCapabilities Test data in the form of a description of a controller's
  /// capabilities. Likely obtained from one of the mappers defined above.
  /// @param [in] expectedDataFormatSpec Test data in the form of a data format spec that is
  /// expected to be the result of creating a data format object using the provided controller
  /// capabilities.
  static void TestDataFormatCreateSuccess(
      const DIDATAFORMAT& appFormatSpec,
      const Controller::SCapabilities controllerCapabilities,
      const DataFormat::SDataFormatSpec& expectedDataFormatSpec)
  {
    // Create the data format object and assert success, then compare the resulting specification
    // object with the expected specification object.
    std::unique_ptr<DataFormat> dataFormat =
        DataFormat::CreateFromApplicationFormatSpec(appFormatSpec, controllerCapabilities);
    ...
    // Iterate through the entire expected specification object and verify that all elements are
    // correctly mapped to and from offsets. This exercises all of the element-mapping methods of
    // the data format object in both directions: element to offset and offset to element. First
    // axes, then buttons, and finally the POV.
    for (int i = 0; i < _countof(expectedDataFormatSpec.axisOffset); ++i)
    {
      const SElementIdentifier expectedAxisIdentifier = {
          .type = EElementType::Axis, .axis = (EAxis)i};
      ...
      {
        const std::optional<SElementIdentifier> maybeActualAxisIdentifier =
            dataFormat->GetElementForOffset(expectedAxisOffset);
        const std::optional<TOffset> maybeActualAxisOffset =
            dataFormat->GetOffsetForElement(expectedAxisIdentifier);
    ...
    {
      const SElementIdentifier expectedButtonIdentifier = {
          .type = EElementType::Button, .button = (EButton)i};
      ...
      {
        const std::optional<SElementIdentifier> maybeActualButtonIdentifier =
            dataFormat->GetElementForOffset(expectedButtonOffset);
        const std::optional<TOffset> maybeActualButtonOffset =
            dataFormat->GetOffsetForElement(expectedButtonIdentifier);
...

    {
      ...
      {
        const std::optional<SElementIdentifier> maybeActualPovIdentifier =
            dataFormat->GetElementForOffset(expectedPovOffset);
        const std::optional<TOffset> maybeActualPovOffset =
            dataFormat->GetOffsetForElement(expectedPovIdentifier);
  ...
  /// Given the information needed to construct a data format object, attempts to construct the data
  /// format object and asserts that the operation fails.
  /// @param [in] appFormatSpec Test data in the form of an application data format specification.
  /// @param [in] controllerCapabilities Test data in the form of a description of a controller's
  /// capabilities. Likely obtained from one of the mappers defined above.
  static void TestDataFormatCreateFailure(
  ...
  TEST_CASE(DataFormat_PovDirectionValue)
  {
    constexpr struct
    {
      ...
      EPovValue expectedPovValue;
        ...
        {.povUp = false,
         ...
         .povLeft = false,
         .povRight = false,
         .expectedPovValue = EPovValue::Center},
        {.povUp = false,
         .povDown = false,
         .povLeft = false,
         .povRight = true,
    ...
    for (const auto& povTestData : kPovTestData)
    {
      ...
      const EPovValue actualPovValue =
          DataFormat::DirectInputPovValue(controllerState.povDirection);
      ...
      if (actualPovValue != expectedPovValue)
      {
        Infra::Test::PrintFormatted(
            ...
            ((true == povTestData.povLeft) ? L"true" : L"false"),
            ((true == povTestData.povRight) ? L"true" : L"false"),
            (int)expectedPovValue,
            (int)actualPovValue);
        numFailingInputs += 1;
      }
    }
  ...
  // Verifies that application data packets are correctly written given both a specification and a
  // controller state. First is a single-element test, in which most of the data packet is zero
  // except for one element from the controller state. Second is an all-element test, in which the
  // entire set of object format specifications is provided and the data format object is expected
  // to write a complete packet.
  TEST_CASE(DataFormat_WriteDataPacket)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        ...
        {.pguid = &GUID_YAxis,
         ...
         .dwFlags = 0},
        {.pguid = &GUID_ZAxis,
         .dwOfs = offsetof(STestDataPacket, axisZ),
         .dwType = DIDFT_AXIS | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
        ...
        {.pguid = nullptr,
         ...
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, button[4]),
         .dwType = DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, button[5]),
         .dwType = DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, button[6]),
         .dwType = DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, button[7]),
         .dwType = DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, extraPov[0]),
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, extraPov[1]),
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, extraPov[2]),
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
    ...
    for (int i = 0; i < _countof(testObjectFormatSpec); ++i)
    {
      ...
      std::unique_ptr<DataFormat> dataFormat = DataFormat::CreateFromApplicationFormatSpec(
          kTestFormatSpec, kTestMapperWithPov.GetCapabilities());
      TEST_ASSERT(nullptr != dataFormat);

      // Generate the expected data packet.
      // Other tests verify element mapping and POV direction value generation, so we can rely on
      // the data format object to perform these tasks in this test. Once the target element is
      // known, extract the correct value from the test controller state object and place it at the
      // right offset within the expected data packet.
      STestDataPacket expectedDataPacket;
      ZeroMemory(&expectedDataPacket, sizeof(expectedDataPacket));

      TEST_ASSERT(
          true == dataFormat->GetElementForOffset(testObjectFormatSpec[i].dwOfs).has_value());
      const SElementIdentifier testElement =
          dataFormat->GetElementForOffset(testObjectFormatSpec[i].dwOfs).value();

      switch (testElement.type)
      {
        case EElementType::Axis:
          *((TAxisValue*)(((size_t)&expectedDataPacket) + (size_t)testObjectFormatSpec[i].dwOfs)) =
              kTestControllerState[testElement.axis];
          break;
        ...
        case EElementType::Pov:
          *((EPovValue*)(((size_t)&expectedDataPacket) + (size_t)testObjectFormatSpec[i].dwOfs)) =
              DataFormat::DirectInputPovValue(kTestControllerState.povDirection);
          break;
      ...
      // Poison-initialize the entire data packet, and then pass the test controller state to the
      // mapper.
      STestDataPacket actualDataPacket;
      FillMemory(&actualDataPacket, sizeof(actualDataPacket), 0xcd);
      TEST_ASSERT(
          true ==
          dataFormat->WriteDataPacket(
              &actualDataPacket, sizeof(actualDataPacket), kTestControllerState));
    ...
    // Logic is the same as the single-element case, except here the expected output is much simpler
    // to generate. Furthermore, unlike the single-element tests, here the extraPov array consists
    // of unused POVs, so it is expected that all elements will be filled with a center position
    // value.
    {
      const DIDATAFORMAT kTestFormatSpec = {
          .dwSize = sizeof(DIDATAFORMAT),
  ...
  // Axis objects are declared in the object specification in increasing offset order, and axes are
  // expected to be selected in the order they appear in the object format specification array.
  TEST_CASE(DataFormat_CreateSuccess_AxisAnyAscending)
  {
    ...
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::X}, offsetof(STestDataPacket, axisValue1));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::Y}, offsetof(STestDataPacket, axisValue2));

    TestDataFormatCreateSuccess(
        kTestFormatSpec, kTestMapperWithPov.GetCapabilities(), expectedDataFormatSpec);
    TestDataFormatCreateSuccess(
        kTestFormatSpec, kTestMapperWithoutPov.GetCapabilities(), expectedDataFormatSpec);
  }

  // Tests a simple data packet with two axis values and allows them to be any type of axis.
  // Axis objects are declared in the object specification in decreasing offset order, so the
  // assignment of axis to offset is inverted compared to the ascending version of this test.
  TEST_CASE(DataFormat_CreateSuccess_AxisAnyDescending)
  {
    ...
    DataFormat::SDataFormatSpec expectedDataFormatSpec(sizeof(STestDataPacket));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::Y}, offsetof(STestDataPacket, axisValue1));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::X}, offsetof(STestDataPacket, axisValue2));
  ...
  // GUID. Axis objects are declared in the object specification in increasing offset order.
  // However, because specific axis types are specified, there is no impact on the assignment of
  // axes to offsets.
  TEST_CASE(DataFormat_CreateSuccess_AxisSpecificByGuidAscending)
  {
    ...
    DataFormat::SDataFormatSpec expectedDataFormatSpec(sizeof(STestDataPacket));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::RotZ}, offsetof(STestDataPacket, axisValue1));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::X}, offsetof(STestDataPacket, axisValue2));
  ...
  // Tests a simple data packet with two axis values, where the specific axis types are specified by
  // GUID. Axis objects are declared in the object specification in decreasing offset order.
  // However, because specific axis types are specified, there is no impact on the assignment of
  // axes to offsets.
  ...
  // Tests a simple data packet with two axis values, where the specific axis types are specified by
  // index into the controller capabilities. Axis objects are declared in the object specification
  // in increasing offset order. However, because specific axis types are specified, there is no
  // impact on the assignment of axes to offsets.
  ...
  // Tests a simple data packet with two axis values, where the specific axis types are specified by
  // index into the controller capabilities. Axis objects are declared in the object specification
  // in decreasing offset order. However, because specific axis types are specified, there is no
  // impact on the assignment of axes to offsets.
  ...
  // "DataFormat.cpp" must be 0. Axis objects are declared in the object specification in decreasing
  ...
  // Tests a simple data packet with three axis values, where each axis is specified such that no
  // controller element matches and so the resulting data format specification is empty. Axis
  // objects are declared in the object specification in decreasing offset order, but the order is
  // immaterial.
  TEST_CASE(DataFormat_CreateSuccess_AxisNoMatch)
  {
    ...
    // Expected to be empty because no elements were able to be selected for any object format
    // specifications.
    DataFormat::SDataFormatSpec expectedDataFormatSpec(sizeof(STestDataPacket));
  ...
  // Button value fields are deliberately numbered out of order so that the object format
  // specification remains readable but the offsets themselves are out of order. See test case body
  // for a description of inputs and expected outputs.
  TEST_CASE(DataFormat_CreateSuccess_Button)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        ...
        {.pguid = &GUID_Button,
         ...
         .dwFlags = DIDOI_ASPECTPOSITION}, // Matches button 1 because it is the next available
        {.pguid = &GUID_Button,
    ...
    // Several, but not all, buttons are expected to match controller elements, based on the
    // specification above.
    DataFormat::SDataFormatSpec expectedDataFormatSpec(sizeof(STestDataPacket));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Button, .button = EButton::B2},
        offsetof(STestDataPacket, buttonValue1));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Button, .button = EButton::B1},
        offsetof(STestDataPacket, buttonValue2));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Button, .button = EButton::B3},
        offsetof(STestDataPacket, buttonValue3));
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Button, .button = EButton::B4},
        offsetof(STestDataPacket, buttonValue7));

    TestDataFormatCreateSuccess(
        kTestFormatSpec, kTestMapperWithPov.GetCapabilities(), expectedDataFormatSpec);

    // Based on the specification above, there are additional situations that match when using the
    // without-POV test data that could not match when using the with-POV test data.
    expectedDataFormatSpec.SetOffsetForElement(
        {.type = EElementType::Button, .button = EButton::B16},
        offsetof(STestDataPacket, buttonValue5));

    TestDataFormatCreateSuccess(
        kTestFormatSpec, kTestMapperWithoutPov.GetCapabilities(), expectedDataFormatSpec);
  }

  // Tests a simple data packet with one POV value whose specification selects it by a type filter
  // value only (i.e. no GUID, and no instance index). A match is expected with the virtual
  // controller that does have a POV, whereas no match is expected with the virtual controller that
  // does not have a POV.
  TEST_CASE(DataFormat_CreateSuccess_PovAny)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, povValue),
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
    };
    ...
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Pov}, offsetof(STestDataPacket, povValue));

    TestDataFormatCreateSuccess(
        kTestFormatSpec, kTestMapperWithPov.GetCapabilities(), expectedDataFormatSpecWithPov);
    ...
    TestDataFormatCreateSuccess(
        kTestFormatSpec, kTestMapperWithoutPov.GetCapabilities(), expectedDataFormatSpecWithoutPov);
  }

  // Tests a simple data packet with one POV value whose specification selects it by a type filter
  // value and GUID but no instance index. A match is expected with the virtual controller that does
  // have a POV, whereas no match is expected with the virtual controller that does not have a POV.
  ...
  // Tests a simple data packet with one POV value whose specification selects it by a type filter
  // value and instance index but no GUID. A match is expected with the virtual controller that does
  // have a POV, whereas no match is expected with the virtual controller that does not have a POV.
  TEST_CASE(DataFormat_CreateSuccess_PovSpecificByIndex)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, povValue),
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_MAKEINSTANCE(0),
         .dwFlags = 0},
    };
  ...
  // Tests a simple data packet with one POV value whose specification selects it by a type filter
  // value, GUID, and instance index. A match is expected with the virtual controller that does have
  // a POV, whereas no match is expected with the virtual controller that does not have a POV.
  ...
  // Tests a simple data packet with one POV value whose specification selects it by a type filter
  // value, GUID, and instance index. An aspect flag is also added. A match is expected with the
  // virtual controller that does have a POV, whereas no match is expected with the virtual
  // controller that does not have a POV.
  ...
  TEST_CASE(DataFormat_CreateSuccess_PovNoMatch)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        ...
        {.pguid = &GUID_Slider,
         ...
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_ANYINSTANCE,
         .dwFlags = DIDOI_ASPECTPOSITION}, // No match because the GUID type does not specify a POV
        {.pguid = &GUID_POV,
    ...
    // Expected to be empty because no elements were able to be selected for any object format
    // specifications. However, because POVs are present in the format specification, they are
    // unused and thus tracked by the data format object. It is assumed that only object
    // specifications recognized as POV specifications (i.e. consistent GUID and type, supported
    // flag value, and so on) need to be tracked as unused POVs.
    DataFormat::SDataFormatSpec expectedDataFormatSpec(sizeof(STestDataPacket));
  ...
  // Applications that use this data format pass the contstant c_dfDIJoystick as a data format
  // specification.
  TEST_CASE(DataFormat_CreateSuccess_DIJoystick)
  {
    ...
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::X}, offsetof(DIJOYSTATE, lX));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::Y}, offsetof(DIJOYSTATE, lY));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::Z}, offsetof(DIJOYSTATE, lZ));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::RotZ}, offsetof(DIJOYSTATE, lRz));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Pov}, offsetof(DIJOYSTATE, rgdwPOV[0]));
    for (int i = 0; i < kTestMapperWithPov.GetCapabilities().numButtons; ++i)
      expectedDataFormatSpecWithPov.SetOffsetForElement(
          {.type = EElementType::Button, .button = (EButton)i},
          offsetof(DIJOYSTATE, rgbButtons[i]));
    ...
    expectedDataFormatSpecWithoutPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::X}, offsetof(DIJOYSTATE, lX));
    expectedDataFormatSpecWithoutPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::Y}, offsetof(DIJOYSTATE, lY));
    expectedDataFormatSpecWithoutPov.SetOffsetForElement(
  ...
  // Applications that use this data format pass the contstant c_dfDIJoystick2 as a data format
  // specification.
  TEST_CASE(DataFormat_CreateSuccess_DIJoystick2)
  {
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        ...
        {.pguid = nullptr,
         ...
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(32),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(33),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(34),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(35),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(36),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(37),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(38),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(39),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(40),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(41),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(42),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(43),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(44),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(45),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(46),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(47),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(48),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(49),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(50),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(51),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(52),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(53),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(54),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(55),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(56),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(57),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(58),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(59),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(60),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(61),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(62),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(63),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(64),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(65),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(66),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(67),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(68),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(69),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(70),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(71),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(72),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(73),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(74),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(75),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(76),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(77),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(78),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(79),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(80),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(81),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(82),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(83),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(84),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(85),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(86),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(87),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(88),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(89),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(90),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(91),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(92),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(93),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(94),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(95),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(96),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(97),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(98),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(99),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(100),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(101),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(102),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(103),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(104),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(105),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(106),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(107),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(108),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(109),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(110),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(111),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(112),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(113),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(114),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(115),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(116),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(117),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(118),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(119),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(120),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(121),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(122),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(123),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(124),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(125),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(126),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = DIJOFS_BUTTON(127),
         .dwType = (DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_XAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lVX),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_YAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lVY),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_ZAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lVZ),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RxAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lVRx),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RyAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lVRy),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RzAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lVRz),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_Slider,
         .dwOfs = offsetof(DIJOYSTATE2, rglVSlider[0]),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_Slider,
         .dwOfs = offsetof(DIJOYSTATE2, rglVSlider[1]),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_XAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lAX),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_YAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lAY),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_ZAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lAZ),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RxAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lARx),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RyAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lARy),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RzAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lARz),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_Slider,
         .dwOfs = offsetof(DIJOYSTATE2, rglASlider[0]),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_Slider,
         .dwOfs = offsetof(DIJOYSTATE2, rglASlider[1]),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_XAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lFX),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_YAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lFY),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_ZAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lFZ),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RxAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lFRx),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RyAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lFRy),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_RzAxis,
         .dwOfs = offsetof(DIJOYSTATE2, lFRz),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_Slider,
         .dwOfs = offsetof(DIJOYSTATE2, rglFSlider[0]),
         .dwType = (DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE),
         .dwFlags = 0},
        {.pguid = &GUID_Slider,
    ...
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::X}, offsetof(DIJOYSTATE2, lX));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::Y}, offsetof(DIJOYSTATE2, lY));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::Z}, offsetof(DIJOYSTATE2, lZ));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Axis, .axis = EAxis::RotZ}, offsetof(DIJOYSTATE2, lRz));
    expectedDataFormatSpecWithPov.SetOffsetForElement(
        {.type = EElementType::Pov}, offsetof(DIJOYSTATE2, rgdwPOV[0]));
    for (int i = 0; i < kTestMapperWithPov.GetCapabilities().numButtons; ++i)
      expectedDataFormatSpecWithPov.SetOffsetForElement(
          {.type = EElementType::Button, .button = (EButton)i},
          offsetof(DIJOYSTATE2, rgbButtons[i]));
  ...
  TEST_CASE(DataFormat_CreateFailure_DataPacketSizeMisaligned)
  {
    ...
    static_assert(
        0 != (sizeof(STestDataPacket) % 4),
        "Test data packet size must not be divisible by 4 for this test.");

    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, buttonValue),
         .dwType = DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
    };
    ...
    const DIDATAFORMAT kTestFormatSpec = {
        ...
        .dwFlags = DIDF_ABSAXIS,
        .dwDataSize = sizeof(
            STestDataPacket), // Reason for rejection: data packet size is not divisible by 4.
        .dwNumObjs = _countof(testObjectFormatSpec),
        .rgodf = testObjectFormatSpec};
  ...
  TEST_CASE(DataFormat_CreateFailure_DataPacketSizeTooLarge)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, buttonValue[0]),
         .dwType = DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
    };
    ...
    const DIDATAFORMAT kTestFormatSpec = {
        ...
        .dwFlags = DIDF_ABSAXIS,
        .dwDataSize =
            sizeof(STestDataPacket), // Reason for rejection: data packet size is too large.
        .dwNumObjs = _countof(testObjectFormatSpec),
        .rgodf = testObjectFormatSpec};
  ...
  TEST_CASE(DataFormat_CreateFailure_NoObjectsDefined)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        {.pguid = nullptr,
         ...
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, buttonValue[1]),
         .dwType = DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, buttonValue[2]),
         .dwType = DIDFT_OPTIONAL | DIDFT_BUTTON | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
  ...
  // Non-optional object without a matching controller element due to an unsupported type filter
  // value.
  ...
  // Non-optional object without a matching controller element due to a GUID filter that does not
  // match any virtual controller elements.
  ...
  // Non-optional object without a matching controller element due to an out-of-bounds instance
  // index value.
  ...
  TEST_CASE(DataFormat_CreateFailure_OffsetOutOfBoundsAxis)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, axisValue[0]),
         .dwType = DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, axisValue[1]),
         .dwType = DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = sizeof(STestDataPacket),
         .dwType = DIDFT_OPTIONAL | DIDFT_AXIS | DIDFT_ANYINSTANCE,
         .dwFlags = 0}, // Reason for rejection: offset exceeds the size of the data packet.
        {.pguid = nullptr,
  ...
  TEST_CASE(DataFormat_CreateFailure_OffsetOutOfBoundsPov)
  {
    ...
    DIOBJECTDATAFORMAT testObjectFormatSpec[] = {
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, povValue[0]),
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = offsetof(STestDataPacket, povValue[1]),
         .dwType = DIDFT_OPTIONAL | DIDFT_POV | DIDFT_ANYINSTANCE,
         .dwFlags = 0},
        {.pguid = nullptr,
         .dwOfs = sizeof(STestDataPacket),
  ...
  // Specified axis offset overlaps with a previous object specification. This is enforced by using
  // a union instead of a struct for the test data packet. Each scenario exercises a different
  // combination of controller elements.
  TEST_CASE(DataFormat_CreateFailure_OffsetOverlap)
  {
    ...
    static_assert(
        0 == (sizeof(UTestDataPacket) % 4), L"Test data packet size must be divisible by 4.");
