 ...
 *   Unit tests for DirectInput interface objects that wrap force feedback effect objects.
 **************************************************************************************************/
...
namespace XidiTest
{
  ...
  protected:
    ...
    SMockTypeSpecificParameters ConvertFromDirectInput(
        const SMockTypeSpecificParameters& diTypeSpecificParams) const override
    {
      return diTypeSpecificParams;
    }

    SMockTypeSpecificParameters ConvertToDirectInput(
        const SMockTypeSpecificParameters& typeSpecificParams) const override
    {
  ...
  /// Creates and returns a smart pointer to a mock physical controller object set up correctly for
  /// force feedback but with neutral state.
  ...
  static inline std::unique_ptr<MockPhysicalController> CreateMockPhysicalController(
      const Mapper& mapper = kTestMapper)
  {
    return std::make_unique<MockPhysicalController>(
        kTestControllerIdentifier, mapper, &kNeutralPhysicalState, 1);
  }

  /// Creates and returns a DirectInput device object that by default uses the mapper at the top of
  /// this file.
  ...
  /// Creates and returns a DirectInput device object that by default uses the mapper and data
  /// packet format at the top of this file. The new object has its data format set and is acquired
  /// in exclusive mode before being returned, so it is immediately ready for force feedback effect
  /// operations.
          ...
          LPCDIDATAFORMAT dataFormatSpec = &kTestFormatSpec)
  {
    ...
    TEST_ASSERT(
        DI_OK ==
        newDirectInputDevice->SetCooperativeLevel(nullptr, DISCL_EXCLUSIVE | DISCL_FOREGROUND));
  ...
  /// @param [in] effectGuid GUID to use to identify the effect, defaults to the test GUID at the
  /// top of this file.
      ...
      const GUID& effectGuid = kTestEffectGuid)
  {
    return std::make_unique<TestVirtualDirectInputEffect>(
        associatedDevice, MockEffectWithTypeSpecificParameters(), effectGuid);
  }
  ...
  // Exercises the nominal situation of creating an effect, setting some parameters, downloading it
  // to the device, starting it, stopping it, and unloading it.
  ...
  TEST_CASE(VirtualDirectInputEffect_ReleaseIsUnload)
  {
    ...
    // For this test it is necessary to manipulate raw pointers just like a real DirectInput
    // application would.
    auto diEffect = CreateTestDirectInputEffect(*diDevice).release();
    const TEffectIdentifier forceFeedbackEffectIdentifier =
        diEffect->UnderlyingEffect().Identifier();
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_AssociatedAxesByOffset)
  {
    ...
    const DIEFFECT effectParameters = {
        ...
        .dwFlags = DIEFF_OBJECTOFFSETS,
        .cAxes = _countof(axes),
        .rgdwAxes = axes};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&effectParameters, (DIEP_AXES | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_AssociatedAxesByNothing)
  {
    ...
    DWORD axes[] = {offsetof(STestDataPacket, axisX), offsetof(STestDataPacket, axisY)};
    const DIEFFECT effectParameters = {
        .dwSize = sizeof(DIEFFECT), .dwFlags = 0, .cAxes = _countof(axes), .rgdwAxes = axes};
    TEST_ASSERT(
        DIERR_INVALIDPARAM ==
        diEffect->SetParametersInternal(&effectParameters, (DIEP_AXES | DIEP_NODOWNLOAD)));
  ...
  // Associated axes, but one of the axes specified does not support force feedback. This should
  // fail.
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_DirectionCartesian)
  {
    ...
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&effectParameters, (DIEP_DIRECTION | DIEP_NODOWNLOAD)));

    TEST_ASSERT(true == ffEffect.HasDirection());
    TEST_ASSERT(2 == ffEffect.Direction().GetNumAxes());

    const std::array<TEffectValue, Controller::ForceFeedback::kEffectAxesMaximumNumber>
        kExpectedCoordinates = {
            (TEffectValue)directionCartesian[0], (TEffectValue)directionCartesian[1]};
    std::array<TEffectValue, Controller::ForceFeedback::kEffectAxesMaximumNumber>
        actualCoordinates = {};
    TEST_ASSERT(
        _countof(directionCartesian) ==
        ffEffect.Direction().GetCartesianCoordinates(
            &actualCoordinates[0], (int)actualCoordinates.size()));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_DirectionPolar)
  {
    ...
    const std::array<TEffectValue, Controller::ForceFeedback::kEffectAxesMaximumNumber>
        kExpectedCoordinates = {(TEffectValue)directionPolar[0]};
    std::array<TEffectValue, Controller::ForceFeedback::kEffectAxesMaximumNumber>
        actualCoordinates = {};
    TEST_ASSERT(
        _countof(directionPolar) ==
        ffEffect.Direction().GetPolarCoordinates(
            &actualCoordinates[0], (int)actualCoordinates.size()));
    TEST_ASSERT(actualCoordinates == kExpectedCoordinates);
  }
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_DirectionSpherical)
  {
    ...
    const DIEFFECT effectParameters = {
        .dwSize = sizeof(DIEFFECT),
        .dwFlags = DIEFF_SPHERICAL,
        .cAxes = 2,
        .rglDirection = directionSpherical};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&effectParameters, (DIEP_DIRECTION | DIEP_NODOWNLOAD)));
    ...
    const std::array<TEffectValue, Controller::ForceFeedback::kEffectAxesMaximumNumber>
        kExpectedCoordinates = {(TEffectValue)directionSpherical[0]};
    std::array<TEffectValue, Controller::ForceFeedback::kEffectAxesMaximumNumber>
        actualCoordinates = {};
    TEST_ASSERT(
        _countof(directionSpherical) ==
        ffEffect.Direction().GetSphericalCoordinates(
            &actualCoordinates[0], (int)actualCoordinates.size()));
    TEST_ASSERT(actualCoordinates == kExpectedCoordinates);
  }
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_Duration)
  {
    ...
    constexpr DIEFFECT kParameters = {
        .dwSize = sizeof(DIEFFECT), .dwDuration = (DWORD)kDuration * 1000};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&kParameters, (DIEP_DURATION | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_Envelope)
  {
    ...
    constexpr SEnvelope kEnvelope = {
        .attackTime = 111, .attackLevel = 222, .fadeTime = 333, .fadeLevel = 444};
    DIENVELOPE diEnvelope = {
        .dwSize = sizeof(DIENVELOPE),
        .dwAttackLevel = (DWORD)kEnvelope.attackLevel,
        .dwAttackTime = (DWORD)kEnvelope.attackTime * 1000,
        .dwFadeLevel = (DWORD)kEnvelope.fadeLevel,
        .dwFadeTime = (DWORD)kEnvelope.fadeTime * 1000};
    const DIEFFECT effectParameters = {.dwSize = sizeof(DIEFFECT), .lpEnvelope = &diEnvelope};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&effectParameters, (DIEP_ENVELOPE | DIEP_NODOWNLOAD)));

    TEST_ASSERT(true == ffEffect.HasEnvelope());
    TEST_ASSERT(kEnvelope == ffEffect.GetEnvelope());

    constexpr DIEFFECT kParametersClearEnvelope = {
        .dwSize = sizeof(DIEFFECT), .lpEnvelope = nullptr};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(
            &kParametersClearEnvelope, (DIEP_ENVELOPE | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_Gain)
  {
    ...
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&kParameters, (DIEP_GAIN | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_SamplePeriod)
  {
    ...
    constexpr DIEFFECT kParameters = {
        .dwSize = sizeof(DIEFFECT), .dwSamplePeriod = (DWORD)kSamplePeriod * 1000};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&kParameters, (DIEP_SAMPLEPERIOD | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_StartDelay)
  {
    ...
    constexpr DIEFFECT kParameters = {
        .dwSize = sizeof(DIEFFECT), .dwStartDelay = (DWORD)kStartDelay * 1000};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&kParameters, (DIEP_STARTDELAY | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_StartDelayIgnored)
  {
    ...
    constexpr TEffectTimeMs kStartDelay = 50000;
    constexpr DIEFFECT kParameters = {
        .dwSize = sizeof(DIEFFECT_DX5), .dwStartDelay = (DWORD)kStartDelay * 1000};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&kParameters, (DIEP_STARTDELAY | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_TypeSpecific)
  {
    ...
    SMockTypeSpecificParameters expectedTypeSpecificParameters = {
        .valid = true, .param1 = 55, .param2 = 6789};
    DIEFFECT parameters = {
        .dwSize = sizeof(DIEFFECT),
        .cbTypeSpecificParams = sizeof(expectedTypeSpecificParameters),
        .lpvTypeSpecificParams = &expectedTypeSpecificParameters};
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED ==
        diEffect->SetParametersInternal(&parameters, (DIEP_TYPESPECIFICPARAMS | DIEP_NODOWNLOAD)));

    TEST_ASSERT(true == ffEffect.HasTypeSpecificParameters());

    SMockTypeSpecificParameters actualTypeSpecificParameters =
        ffEffect.GetTypeSpecificParameters().value();
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_InvalidTypeSpecificParameters)
  {
    ...
    SMockTypeSpecificParameters invalidTypeSpecificParameters = {
        .valid = false, .param1 = 1000, .param2 = 2000};
    DIEFFECT parameters = {
        .dwSize = sizeof(DIEFFECT),
        .cbTypeSpecificParams = sizeof(invalidTypeSpecificParameters),
        .lpvTypeSpecificParams = &invalidTypeSpecificParameters};
    TEST_ASSERT(
        DIERR_INVALIDPARAM ==
        diEffect->SetParametersInternal(&parameters, (DIEP_TYPESPECIFICPARAMS | DIEP_NODOWNLOAD)));
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_InvalidFixableTypeSpecificParameters)
  {
    ...
    const SMockTypeSpecificParameters expectedTypeSpecificParameters = {
        .valid = true,
        .param1 = invalidTypeSpecificParameters.param1,
        .param2 = invalidTypeSpecificParameters.param2};
    const SMockTypeSpecificParameters actualTypeSpecificParameters =
        ffEffect.GetTypeSpecificParameters().value();
    TEST_ASSERT(actualTypeSpecificParameters == expectedTypeSpecificParameters);
  }

  // Specifies a complete set of parameters and automatically downloads, but does not start, the
  // effect.
  TEST_CASE(VirtualDirectInputEffect_SetParameters_CompleteAndDownload)
  {
    ...
    TEST_ASSERT(
        ...
        diEffect->SetParametersInternal(
            &effectParameters,
            (DIEP_DURATION | DIEP_AXES | DIEP_DIRECTION | DIEP_TYPESPECIFICPARAMS),
            0));
    TEST_ASSERT(true == ffDevice.IsEffectOnDevice(ffEffect.Identifier()));
    TEST_ASSERT(false == ffDevice.IsEffectPlaying(ffEffect.Identifier()));
  }
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_CompleteAndStart)
  {
    ...
    TEST_ASSERT(
        ...
        diEffect->SetParametersInternal(
            &effectParameters,
            (DIEP_DURATION | DIEP_AXES | DIEP_DIRECTION | DIEP_TYPESPECIFICPARAMS | DIEP_START),
            0));
    TEST_ASSERT(true == ffDevice.IsEffectOnDevice(ffEffect.Identifier()));
    TEST_ASSERT(true == ffDevice.IsEffectPlaying(ffEffect.Identifier()));
  }

  // Specifies an empty set of new parameters and with the download operation skipped, so nothing
  // should happen.
  TEST_CASE(VirtualDirectInputEffect_SetParameters_Empty)
  {
    ...
    TEST_ASSERT(
        DI_DOWNLOADSKIPPED == diEffect->SetParametersInternal(&kEffectParameters, DIEP_NODOWNLOAD));
    TEST_ASSERT(false == ffDevice.IsEffectOnDevice(ffEffect.Identifier()));
  }
  ...
  TEST_CASE(VirtualDirectInputEffect_SetParameters_TooManyBehaviorFlags)
  {
    ...
    TEST_ASSERT(
        DIERR_INVALIDPARAM ==
        diEffect->SetParametersInternal(
            &kEffectParameters, DIEP_NODOWNLOAD | DIEP_NORESTART | DIEP_START));
  }
  ...
  // Associated axes but with a buffer size that is too small. This should retrieve the correct
  // buffer size.
  ...
  TEST_CASE(VirtualDirectInputEffect_GetParameters_DirectionCartesian)
  {
    ...
    TEST_ASSERT(
        true ==
        ffEffect.Direction().SetDirectionUsingCartesian(
            kExpectedCoordinates, _countof(kExpectedCoordinates)));

    LONG actualCoordinates[_countof(kExpectedCoordinates)] = {};
    DIEFFECT parameters = {
        .dwSize = sizeof(DIEFFECT),
        .dwFlags = DIEFF_CARTESIAN,
        .cAxes = _countof(actualCoordinates),
        .rglDirection = actualCoordinates};
  ...
  // Direction in Cartesian coordinates but with the expectation that DirectInput will pick this
  // coordinate system because it was the original one used.
  TEST_CASE(VirtualDirectInputEffect_GetParameters_DirectionOriginalCartesian)
  {
    ...
    constexpr DWORD kInputFlags =
        (DIEFF_CARTESIAN | DIEFF_POLAR | DIEFF_SPHERICAL | DIEFF_OBJECTIDS);
  ...
  TEST_CASE(VirtualDirectInputEffect_GetParameters_DirectionPolar)
  {
    ...
    TEST_ASSERT(
        true ==
        ffEffect.Direction().SetDirectionUsingPolar(
            kExpectedCoordinates, _countof(kExpectedCoordinates) - 1));
  ...
  // Direction in polar coordinates but with the expectation that DirectInput will pick this
  // coordinate system because it was the original one used.
  ...
  TEST_CASE(VirtualDirectInputEffect_GetParameters_DirectionSpherical)
  {
    ...
    TEST_ASSERT(
        true ==
        ffEffect.Direction().SetDirectionUsingSpherical(
            kExpectedCoordinates, _countof(kExpectedCoordinates) - 1));
  ...
  // Direction in spherical coordinates but with the expectation that DirectInput will pick this
  // coordinate system because it was the original one used.
  TEST_CASE(VirtualDirectInputEffect_GetParameters_DirectionOriginalSpherical)
  {
    ...
    constexpr DWORD kInputFlags =
        (DIEFF_CARTESIAN | DIEFF_POLAR | DIEFF_SPHERICAL | DIEFF_OBJECTOFFSETS);
  ...
  // Direction but with a buffer size that is too small. This should retrieve the correct buffer
  // size.
  TEST_CASE(VirtualDirectInputEffect_GetParameters_DirectionInsufficientBuffer)
  {
    ...
    DIEFFECT parameters = {
        .dwSize = sizeof(DIEFFECT), .dwFlags = kInputFlags, .cAxes = 0, .rglDirection = nullptr};
  ...
  TEST_CASE(VirtualDirectInputEffect_GetParameters_Envelope)
  {
    ...
    constexpr SEnvelope kInputEnvelope = {
        .attackTime = 111, .attackLevel = 222, .fadeTime = 333, .fadeLevel = 444};
    constexpr DIENVELOPE kExpectedEnvelope = {
        .dwSize = sizeof(DIENVELOPE),
        .dwAttackLevel = (DWORD)kInputEnvelope.attackLevel,
        .dwAttackTime = (DWORD)kInputEnvelope.attackTime * 1000,
        .dwFadeLevel = (DWORD)kInputEnvelope.fadeLevel,
        .dwFadeTime = (DWORD)kInputEnvelope.fadeTime * 1000};
  ...
  TEST_CASE(VirtualDirectInputEffect_GetParameters_TypeSpecific)
  {
    ...
    constexpr SMockTypeSpecificParameters kExpectedTypeSpecificParameters = {
        .valid = true, .param1 = 1234, .param2 = 5678};
    TEST_ASSERT(true == ffEffect.SetTypeSpecificParameters(kExpectedTypeSpecificParameters));

    SMockTypeSpecificParameters actualTypeSpecificParameters = {};
    DIEFFECT parameters = {
        .dwSize = sizeof(DIEFFECT),
        .cbTypeSpecificParams = sizeof(actualTypeSpecificParameters),
        .lpvTypeSpecificParams = &actualTypeSpecificParameters};
  ...
  // Type-specific parameters but with a buffer size that is too small. This should retrieve the
  // correct buffer size.
  TEST_CASE(VirtualDirectInputEffect_GetParameters_TypeSpecificInsufficientBuffer)
  {
    ...
    constexpr SMockTypeSpecificParameters kTypeSpecificParameters = {
        .valid = true, .param1 = 1234, .param2 = 5678};
    TEST_ASSERT(true == ffEffect.SetTypeSpecificParameters(kTypeSpecificParameters));

    DIEFFECT parameters = {
        .dwSize = sizeof(DIEFFECT), .cbTypeSpecificParams = 0, .lpvTypeSpecificParams = nullptr};
