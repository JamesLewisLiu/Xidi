...
namespace XidiTest
{
  ...
  /// Only works for simple element mappers that uniquely target zero or one specific controller
  /// elements and have no side effects.
  ...
  static void VerifyElementMapperPointersAreEquivalent(
      const IElementMapper* elementMapperA, const IElementMapper* elementMapperB)
  {
    ...
    else
    {
      TEST_ASSERT(nullptr != elementMapperB);
      TEST_ASSERT(
          elementMapperA->GetTargetElementCount() == elementMapperB->GetTargetElementCount());
  ...
  /// Checks if the supplied element mapper parse results are equivalent and flags a test failure if
  /// not. Only works for simple element mappers that uniquely target zero or one specific
  /// controller elements and have no side effects.
  ...
  static void VerifyParseResultsAreEquivalent(
      const SElementMapperParseResult& resultA, const SElementMapperParseResult& resultB)
  {
    ...
    if (resultA.maybeElementMapper.HasValue())
      VerifyElementMapperPointersAreEquivalent(
          resultA.maybeElementMapper.Value().get(), resultB.maybeElementMapper.Value().get());
  }
  ...
  static inline void VerifySplitMapperPointersAreEquivalent(
      const SplitMapper* splitMapperA, const SplitMapper* splitMapperB)
  {
    ...
    {
      const IElementMapper* subMappersA[] = {
          splitMapperA->GetPositiveMapper(), splitMapperA->GetNegativeMapper()};
      const IElementMapper* subMappersB[] = {
          splitMapperB->GetPositiveMapper(), splitMapperB->GetNegativeMapper()};
      static_assert(
          _countof(subMappersA) == _countof(subMappersB), "Sub mapper array size mismatch.");
  ...
  TEST_CASE(MapperParser_ControllerElementString_Valid)
  {
    ...
    for (const auto& controllerElement : kControllerElements)
    {
      ...
      TEST_ASSERT(
          controllerElement.first ==
          MapperParser::FindControllerElementIndex(controllerElement.second));
    }
  }
  ...
  {
    constexpr std::wstring_view kControllerElementStrings[] = {
        L"stickLeftZ", L"dpadDown", L"random_string"};
    ...
    for (auto controllerElementString : kControllerElementStrings)
    {
      ...
      TEST_ASSERT(
          false == MapperParser::FindControllerElementIndex(controllerElementString).has_value());
    }
  }
  ...
  TEST_CASE(MapperParser_ForceFeedbackActuatorString_Valid)
  {
    ...
    for (const auto& ffActuator : kForceFeedbackActuators)
    {
      ...
      TEST_ASSERT(
          ffActuator.first == MapperParser::FindForceFeedbackActuatorIndex(ffActuator.second));
    }
  }
  ...
  {
    constexpr std::wstring_view kForceFeedbackActuatorStrings[] = {
        L"leftMotor", L"RightMotor", L"random_string"};
    ...
    for (auto ffActuatorString : kForceFeedbackActuatorStrings)
    {
      ...
      TEST_ASSERT(
          false == MapperParser::FindForceFeedbackActuatorIndex(ffActuatorString).has_value());
    }
  }

  // Verifies correct determination of recursion depth, given a set of input strings that are all
  // properly balanced.
  TEST_CASE(MapperParser_RecursionDepth_Balanced)
  {
    ...
    for (auto& recursionTestItem : kRecursionTestItems)
      TEST_ASSERT(
          recursionTestItem.first == MapperParser::ComputeRecursionDepth(recursionTestItem.second));
  }

  // Verifies inability to compute recursion depth, given a set of input strings that are not
  // properly balanced.
  ...
  // Verifies correct separation of an input element mapper string into type and parameter
  // substrings. Exercises several different simple cases in which the element mapper string
  // contains one type and one set of parameters. The whole string is consumed, so there is no
  // remainder.
  TEST_CASE(MapperParser_ExtractElementMapperStringParts_Simple)
  {
    ...
    for (auto& extractPartsTestItem : kExtractPartsTestItems)
      TEST_ASSERT(
          extractPartsTestItem.second ==
          MapperParser::ExtractElementMapperStringParts(extractPartsTestItem.first));
  }

  // Verifies correct separation of an input element mapper string into type and parameter
  // substrings. Exercises several different nested cases in which an element mapper string has
  // other element mapper strings as parameters. The whole string is consumed, so there is no
  // remainder.
  ...
  // Verifies correct separation of an input mapper element string into type, parameter, and
  // remaining substrings. Exercises situations in which the whole string is not consumed, so there
  // is a remaining part of the string left behind.
  ...
  // Verifies correct rejection of invalid element mapper strings when attempting to split into
  // type, parameter, and remaining substrings.
  TEST_CASE(MapperParser_ExtractElementMapperStringParts_Invalid)
  {
    ...
    for (auto& extractPartsTestString : kExtractPartsTestStrings)
      TEST_ASSERT(
          false ==
          MapperParser::ExtractElementMapperStringParts(extractPartsTestString).has_value());
  }
  ...
  TEST_CASE(MapperParser_ExtractForceFeedbackActuatorStringParts_Valid)
  {
    ...
    for (auto& extractPartsTestItem : kExtractPartsTestItems)
      TEST_ASSERT(
          extractPartsTestItem.second ==
          MapperParser::ExtractForceFeedbackActuatorStringParts(extractPartsTestItem.first));
  }
  ...
  TEST_CASE(MapperParser_ExtractForceFeedbackActuatorStringParts_Invalid)
  {
    ...
    for (auto& extractPartsTestString : kExtractPartsTestStrings)
      TEST_ASSERT(
          false ==
          MapperParser::ExtractForceFeedbackActuatorStringParts(extractPartsTestString)
              .has_value());
  }

  // Verifies correct separation of a parameter string into first parameter and remainder
  // substrings.
  TEST_CASE(MapperParser_ExtractParameterListStringParts_Valid)
  {
    ...
    for (auto& extractPartsTestItem : kExtractPartsTestItems)
      TEST_ASSERT(
          extractPartsTestItem.second ==
          MapperParser::ExtractParameterListStringParts(extractPartsTestItem.first));
  }

  // Verifies correct rejection of invalid parameter list strings when attempting to split into
  // first parameter and remainder substrings.
  TEST_CASE(MapperParser_ExtractParameterListStringParts_Invalid)
  {
    ...
    for (auto& extractPartsTestString : kExtractPartsTestStrings)
      TEST_ASSERT(
          false ==
          MapperParser::ExtractParameterListStringParts(extractPartsTestString).has_value());
  }

  // Verifies correct construction of axis mapper objects in the nominal case of valid parameter
  // strings being passed. This test does not check axis directon, just target virtual controller
  // element.
  ...
  {
    const std::wstring_view kAxisMapperTestStrings[] = {
        L"A", L"3", L"x, anydir", L"rotz, +, morestuff"};
  ...
  // Verifies correct construction of button mapper objects in the nominal case of valid parameter
  ...
  TEST_CASE(MapperParser_MakeButtonMapper_Nominal)
  {
    ...
    {
      ElementMapperOrError maybeButtonMapper =
          MapperParser::MakeButtonMapper(buttonMapperTestItem.first);
  ...
  // Verifies correct failure to create button mapper objects when the parameter strings are
  // invalid.
  TEST_CASE(MapperParser_MakeButtonMapper_Invalid)
  {
    ...
    {
      ElementMapperOrError maybeButtonMapper =
          MapperParser::MakeButtonMapper(buttonMapperTestString);
      TEST_ASSERT(false == maybeButtonMapper.HasValue());
    }
  }

  // Verifies correct construction of compound mapper objects in the nominal case of valid parameter
  // strings being passed. In this test only simple underlying element mapper types are used.
  TEST_CASE(MapperParser_MakeCompoundMapper_Nominal)
  {
    ...
    {
      ElementMapperOrError maybeCompoundMapper =
          MapperParser::MakeCompoundMapper(kCompoundMapperTestStrings[i]);

      TEST_ASSERT(true == maybeCompoundMapper.HasValue());
      TEST_ASSERT(
          expectedElements[i].size() == maybeCompoundMapper.Value()->GetTargetElementCount());

      for (unsigned int j = 0; j < expectedElements[i].size(); ++j)
      {
        const SElementIdentifier expectedElement = expectedElements[i][j];

        const std::optional<SElementIdentifier> maybeActualElement =
            maybeCompoundMapper.Value()->GetTargetElementAt(j);
  ...
  // Verifies correct failure to create compound mapper objects when the parameter strings are
  // invalid.
  TEST_CASE(MapperParser_MakeCompoundMapper_Invalid)
  {
    ...
    {
      ElementMapperOrError maybeCompoundMapper =
          MapperParser::MakeCompoundMapper(compoundMapperTestString);
      TEST_ASSERT(false == maybeCompoundMapper.HasValue());
    }
  }

  // Verifies correct construction of digital axis mapper objects in the nominal case of valid
  // parameter strings being passed. Same as the corresponding axis mapper test but with a different
  // target type.
  TEST_CASE(MapperParser_MakeDigitalAxisMapper_Nominal)
  {
    ...
    {
      ElementMapperOrError maybeDigitalAxisMapper =
          MapperParser::MakeDigitalAxisMapper(digitalAxisMapperTestItem.first);

      TEST_ASSERT(true == maybeDigitalAxisMapper.HasValue());
      TEST_ASSERT(1 == maybeDigitalAxisMapper.Value()->GetTargetElementCount());
      TEST_ASSERT(
          digitalAxisMapperTestItem.second ==
          maybeDigitalAxisMapper.Value()->GetTargetElementAt(0));
    }
  }

  // Verifies correct failure to create digital axis mapper objects when the parameter strings are
  // invalid. Same as the corresponding axis mapper test but with a different target type.
  TEST_CASE(MapperParser_MakeDigitalAxisMapper_Invalid)
  {
    const std::wstring_view kDigitalAxisMapperTestStrings[] = {
        L"A", L"3", L"x, anydir", L"rotz, +, morestuff"};

    for (auto& digitalAxisMapperTestString : kDigitalAxisMapperTestStrings)
    {
      ElementMapperOrError maybeDigitalAxisMapper =
          MapperParser::MakeDigitalAxisMapper(digitalAxisMapperTestString);
      TEST_ASSERT(false == maybeDigitalAxisMapper.HasValue());
    }
  }

  // Verifies correct construction of invert mapper objects in the nominal case of using very simple
  // non-null inner element mappers represented by valid strings.
  TEST_CASE(MapperParser_MakeInvertMapper_Nominal)
  {
    constexpr std::wstring_view kInvertMapperTestStrings[] = {
        L"Axis(X)", L" Pov(  Up  )", L"   Button(10) ", L"Axis(RotX, +)"};
    ...
    static_assert(
        _countof(expectedElements) == _countof(kInvertMapperTestStrings),
        "Mismatch between input and expected output array lengths.");

    for (int i = 0; i < _countof(kInvertMapperTestStrings); ++i)
    {
      ElementMapperOrError maybeInvertMapper =
          MapperParser::MakeInvertMapper(kInvertMapperTestStrings[i]);
  ...
  // Verifies correct failure to create invert mapper objects when the parameter strings are
  // invalid.
  TEST_CASE(MapperParser_MakeInvertMapper_Invalid)
  {
    constexpr std::wstring_view kInvertMapperTestStrings[] = {
        L"Axis(X), Axis(RotX), Axis(Z)", L"Button(100)", L"Null, Null"};

    for (auto& invertMapperTestString : kInvertMapperTestStrings)
    {
      ElementMapperOrError maybeInvertMapper =
          MapperParser::MakeInvertMapper(invertMapperTestString);
      TEST_ASSERT(false == maybeInvertMapper.HasValue());
    }
  }

  // Verifies correct construction of keyboard mapper objects in the nominal case of valid parameter
  // strings being passed.
  TEST_CASE(MapperParser_MakeKeyboardMapper_Nominal)
  {
    ...
    {
      ElementMapperOrError maybeKeyboardMapper =
          MapperParser::MakeKeyboardMapper(keyboardMapperTestItem.first);
      ...
      const TKeyIdentifier actualTargetKey =
          dynamic_cast<KeyboardMapper*>(maybeKeyboardMapper.Value().get())->GetTargetKey();
      TEST_ASSERT(actualTargetKey == expectedTargetKey);
    }
  }

  // Verifies correct failure to create keyboard mapper objects when the parameter strings are
  // invalid.
  TEST_CASE(MapperParser_MakeKeyboardMapper_Invalid)
  {
    const std::wstring_view kKeyboardMapperTestStrings[] = {
        L"256", L"0x101", L"DIK_INVALID", L"Invalid", L""};

    for (auto& keyboardMapperTestString : kKeyboardMapperTestStrings)
    {
      ElementMapperOrError maybeKeyboardMapper =
          MapperParser::MakeKeyboardMapper(keyboardMapperTestString);
      TEST_ASSERT(false == maybeKeyboardMapper.HasValue());
    }
  }

  // Verifies correct construction of mouse axis mapper objects in the nominal case of valid
  // parameter strings being passed.
  TEST_CASE(MapperParser_MakeMouseAxisMapper_Nominal)
  {
    ...
    {
      ElementMapperOrError maybeMouseAxisMapper =
          MapperParser::MakeMouseAxisMapper(mouseAxisMapperTestItem.params);

      TEST_ASSERT(true == maybeMouseAxisMapper.HasValue());
      TEST_ASSERT(0 == maybeMouseAxisMapper.Value()->GetTargetElementCount());

      const MouseAxisMapper* const mouseAxisMapper =
          dynamic_cast<MouseAxisMapper*>(maybeMouseAxisMapper.Value().get());
  ...
  // Verifies correct failure to create mouse button mapper objects when the parameter strings are
  // invalid.
  TEST_CASE(MapperParser_MakeMouseAxisMapper_Invalid)
  {
    const std::wstring_view kMouseAxisMapperTestStrings[] = {
        L"B", L"5", L"x, anydir", L"wheelX, +, morestuff"};

    for (auto& mouseAxisMapperTestString : kMouseAxisMapperTestStrings)
    {
      ElementMapperOrError maybeMouseAxisMapper =
          MapperParser::MakeMouseAxisMapper(mouseAxisMapperTestString);
      TEST_ASSERT(false == maybeMouseAxisMapper.HasValue());
    }
  }

  // Verifies correct construction of mouse button mapper objects in the nominal case of valid
  // parameter strings being passed.
  TEST_CASE(MapperParser_MakeMouseButtonMapper_Nominal)
  {
    ...
    {
      ElementMapperOrError maybeMouseButtonMapper =
          MapperParser::MakeMouseButtonMapper(mouseButtonMapperTestItem.first);

      TEST_ASSERT(true == maybeMouseButtonMapper.HasValue());
      TEST_ASSERT(0 == maybeMouseButtonMapper.Value()->GetTargetElementCount());

      TEST_ASSERT(
          nullptr != dynamic_cast<MouseButtonMapper*>(maybeMouseButtonMapper.Value().get()));

      const EMouseButton expectedTargetMouseButton = mouseButtonMapperTestItem.second;
      const EMouseButton actualTargetMouseButton =
          dynamic_cast<MouseButtonMapper*>(maybeMouseButtonMapper.Value().get())->GetMouseButton();
  ...
  TEST_CASE(MapperParser_MakeMouseButtonMapper_Invalid)
  {
    ...
    {
      ElementMapperOrError maybeMouseButtonMapper =
          MapperParser::MakeKeyboardMapper(mouseButtonMapperTestString);
      TEST_ASSERT(false == maybeMouseButtonMapper.HasValue());
    }
  }

  // Verifies correct construction of null mappers in the nominal case of empty parameter strings
  // being passed.
  ...
  // Verifies correct construction of POV mapper objects in the nominal case of valid parameter
  // strings being passed. This test does not check POV directon, just target virtual controller
  // element.
  ...
  // Verifies correct construction of split mapper objects in the nominal case of using very simple
  // non-null inner element mappers represented by valid strings.
  TEST_CASE(MapperParser_MakeSplitMapper_Nominal)
  {
    ...
    {
      ElementMapperOrError maybeSplitMapper =
          MapperParser::MakeSplitMapper(kSplitMapperTestStrings[i]);
  ...
  {
    constexpr std::wstring_view kSplitMapperTestStrings[] = {
        L"Axis(X), Axis(RotX), Axis(Z)", L"Button(10)", L"Null, Null, Null, Null, Null"};
  ...
  // Verifies correct construction of split mapper objects when both inner mappers are represented
  // by valid strings but one is null.
  TEST_CASE(MapperParser_MakeSplitMapper_OneNull)
  {
    ...
    {
      ElementMapperOrError maybeSplitMapper =
          MapperParser::MakeSplitMapper(splitMapperTestItem.first);
  ...
  TEST_CASE(MapperParser_ParseSingleElementMapper_Axis)
  {
    constexpr std::wstring_view kTestStrings[] = {
        L"Axis(X)", L"Axis(Y, Both)", L"Axis(Z, +)", L"Axis(RX, negative), Button(3)"};
    const SElementMapperParseResult expectedParseResults[] = {
        ...
        {.maybeElementMapper = std::make_unique<AxisMapper>(EAxis::RotX, EAxisDirection::Negative),
    ...
    {
      SElementMapperParseResult actualParseResult =
          MapperParser::ParseSingleElementMapper(kTestStrings[i]);
      VerifyParseResultsAreEquivalent(actualParseResult, expectedParseResults[i]);

      TEST_ASSERT(
          nullptr != dynamic_cast<AxisMapper*>(actualParseResult.maybeElementMapper.Value().get()));

      const EAxisDirection expectedDirection =
          dynamic_cast<AxisMapper*>(expectedParseResults[i].maybeElementMapper.Value().get())
              ->GetAxisDirection();
      const EAxisDirection actualDirection =
          dynamic_cast<AxisMapper*>(actualParseResult.maybeElementMapper.Value().get())
              ->GetAxisDirection();
  ...
  {
    constexpr std::wstring_view kTestStrings[] = {
        L"Button(10)", L"  Button   (    10   )  ", L"Button(1), Button(3)"};
    const SElementMapperParseResult expectedParseResults[] = {
  ...
  TEST_CASE(MapperParser_ParseSingleElementMapper_Compound)
  {
    ...
    const SElementMapperParseResult expectedParseResults[] = {
        ...
        {.maybeElementMapper = std::make_unique<CompoundMapper>(CompoundMapper::TElementMappers(
             {std::make_unique<InvertMapper>(std::make_unique<AxisMapper>(EAxis::X)),
    ...
    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      ...
      TEST_ASSERT(
          nullptr !=
          dynamic_cast<CompoundMapper*>(actualParseResult.maybeElementMapper.Value().get()));

      const CompoundMapper::TElementMappers& expectedElementMappers =
          dynamic_cast<CompoundMapper*>(expectedParseResults[i].maybeElementMapper.Value().get())
              ->GetElementMappers();
      const CompoundMapper::TElementMappers& actualElementMappers =
          dynamic_cast<CompoundMapper*>(actualParseResult.maybeElementMapper.Value().get())
              ->GetElementMappers();

      for (size_t j = 0; j < expectedElementMappers.size(); ++j)
        VerifyElementMapperPointersAreEquivalent(
            actualElementMappers[j].get(), expectedElementMappers[j].get());
    }
  }

  // Verifies correct parsing of single digital axis element mappers from a valid supplied input
  // string. Same as the axis mapper test but with a different target type.
  TEST_CASE(MapperParser_ParseSingleElementMapper_DigitalAxis)
  {
    ...
    const SElementMapperParseResult expectedParseResults[] = {
        ...
        {.maybeElementMapper =
             std::make_unique<DigitalAxisMapper>(EAxis::Z, EAxisDirection::Positive)},
        {.maybeElementMapper =
             std::make_unique<DigitalAxisMapper>(EAxis::RotX, EAxisDirection::Negative),
         .remainingString = L"Button(3)"},
    };
    static_assert(
        _countof(expectedParseResults) == _countof(kTestStrings),
    ...
    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      ...
      TEST_ASSERT(
          nullptr !=
          dynamic_cast<DigitalAxisMapper*>(actualParseResult.maybeElementMapper.Value().get()));

      const EAxisDirection expectedDirection =
          dynamic_cast<DigitalAxisMapper*>(expectedParseResults[i].maybeElementMapper.Value().get())
              ->GetAxisDirection();
      const EAxisDirection actualDirection =
          dynamic_cast<DigitalAxisMapper*>(actualParseResult.maybeElementMapper.Value().get())
              ->GetAxisDirection();
  ...
  {
    constexpr std::wstring_view kTestStrings[] = {
        L"Invert(Axis(X))", L"Invert(Pov(Right))", L"Invert( Split(Button(10), Button(12)) )"};
    const SElementMapperParseResult expectedParseResults[] = {
        {.maybeElementMapper =
             std::make_unique<InvertMapper>(std::make_unique<AxisMapper>(EAxis::X))},
        {.maybeElementMapper =
             std::make_unique<InvertMapper>(std::make_unique<PovMapper>(EPovDirection::Right))},
    ...
    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      ...
      TEST_ASSERT(
          nullptr !=
          dynamic_cast<InvertMapper*>(actualParseResult.maybeElementMapper.Value().get()));

      const IElementMapper* expectedElementMapper =
          expectedParseResults[i].maybeElementMapper.Value().get();
      const IElementMapper* actualElementMapper =
          dynamic_cast<InvertMapper*>(actualParseResult.maybeElementMapper.Value().get())
              ->GetElementMapper();
  ...
  TEST_CASE(MapperParser_ParseSingleElementMapper_Keyboard)
  {
    ...
    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      ...
      TEST_ASSERT(
          nullptr !=
          dynamic_cast<KeyboardMapper*>(actualParseResult.maybeElementMapper.Value().get()));

      const TKeyIdentifier expectedTargetKey =
          dynamic_cast<KeyboardMapper*>(expectedParseResults[i].maybeElementMapper.Value().get())
              ->GetTargetKey();
      const TKeyIdentifier actualTargetKey =
          dynamic_cast<KeyboardMapper*>(actualParseResult.maybeElementMapper.Value().get())
              ->GetTargetKey();
  ...
  {
    constexpr std::wstring_view kTestStrings[] = {L"PovHat(Up)", L"Pov(Left)", L"POV(Dn)"};
    const SElementMapperParseResult expectedParseResults[] = {
    ...
    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      ...
      TEST_ASSERT(
          nullptr != dynamic_cast<PovMapper*>(actualParseResult.maybeElementMapper.Value().get()));

      const EPovDirection expectedDirection =
          dynamic_cast<PovMapper*>(expectedParseResults[i].maybeElementMapper.Value().get())
              ->GetDirection();
      const EPovDirection actualDirection =
          dynamic_cast<PovMapper*>(actualParseResult.maybeElementMapper.Value().get())
              ->GetDirection();
      TEST_ASSERT(actualDirection == expectedDirection);
    }
  }
  ...
  {
    constexpr std::wstring_view kTestStrings[] = {
        L"Null", L"  Null  ", L"  Null  , Null  , Button(2) "};
    const SElementMapperParseResult expectedParseResults[] = {
  ...
  // Verifies correct parsing of single split element mappers of varying complexity from a valid
  ...
  TEST_CASE(MapperParser_ParseSingleElementMapper_Split)
  {
    ...
    const SElementMapperParseResult expectedParseResults[] = {
        {.maybeElementMapper = std::make_unique<SplitMapper>(
             std::make_unique<AxisMapper>(EAxis::X), std::make_unique<AxisMapper>(EAxis::Y))},
        {.maybeElementMapper = std::make_unique<SplitMapper>(
             ...
             std::make_unique<SplitMapper>(
                 ...
                 std::make_unique<ButtonMapper>(EButton::B4)))},
        {.maybeElementMapper = std::make_unique<SplitMapper>(
             std::make_unique<SplitMapper>(nullptr, std::make_unique<ButtonMapper>(EButton::B2)),
             nullptr)},
        {.maybeElementMapper = std::make_unique<SplitMapper>(nullptr, nullptr)}};
    static_assert(
        _countof(expectedParseResults) == _countof(kTestStrings),
        "Mismatch between input and expected output array lengths.");
    ...
    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      ...
      TEST_ASSERT(
          nullptr !=
          dynamic_cast<SplitMapper*>(actualParseResult.maybeElementMapper.Value().get()));

      const SplitMapper* const expectedSplitMapper =
          dynamic_cast<SplitMapper*>(expectedParseResults[i].maybeElementMapper.Value().get());
      const SplitMapper* const actualSplitMapper =
          dynamic_cast<SplitMapper*>(actualParseResult.maybeElementMapper.Value().get());
  ...
  TEST_CASE(MapperParser_ParseSingleElementMapper_Invalid)
  {
    constexpr std::wstring_view kTestStrings[] = {
        ...
        L"Pov(Up, Left, Right)",
        L"Pov(AnyDir)"};
    const SElementMapperParseResult expectedParseResult = {
        .maybeElementMapper = L"Test error message."};
  ...
  {
    constexpr std::wstring_view kTestStrings[] = {
        L"Null", L"  Null  ", L"Button(3)", L"  Button   (    5    )  "};
    const std::unique_ptr<IElementMapper> expectedElementMappers[] = {
        ...
        std::make_unique<ButtonMapper>(EButton::B5)};
    static_assert(
        _countof(expectedElementMappers) == _countof(kTestStrings),
        "Mismatch between input and expected output array lengths.");

    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      const ElementMapperOrError maybeActualElementMapper =
          MapperParser::ElementMapperFromString(kTestStrings[i]);
      TEST_ASSERT(true == maybeActualElementMapper.HasValue());
      VerifyElementMapperPointersAreEquivalent(
          maybeActualElementMapper.Value().get(), expectedElementMappers[i].get());
    }
  }
  ...
  {
    constexpr std::wstring_view kTestStrings[] = {
        L"Split(Null, Null)", L"Split(Split(Null, Null), Split(Button(3), Button(4)))"};
    const std::unique_ptr<IElementMapper> expectedElementMappers[] = {
        std::make_unique<SplitMapper>(nullptr, nullptr),
        std::make_unique<SplitMapper>(
            std::make_unique<SplitMapper>(nullptr, nullptr),
            std::make_unique<SplitMapper>(
                std::make_unique<ButtonMapper>(EButton::B3),
  ...
  {
    constexpr std::wstring_view kTestStrings[] = {
        L"Null, Null", L"  UnknownMapperType  ", L"Button(3), Button(4)", L"Button((8))"};
  ...
  // Verifies failure to parse element mapper strings that are syntactically correct but have
  ...
  // Verifies successful parsing of force feedback actuator strings to force feedback actuator
  // description objects.
  TEST_CASE(MapperParser_ForceFeedbackActuatorFromString_Valid)
  {
    ...
    for (int i = 0; i < _countof(kTestStrings); ++i)
    {
      const ForceFeedbackActuatorOrError maybeActualForceFeedbackActuator =
          MapperParser::ForceFeedbackActuatorFromString(kTestStrings[i]);
