 ...
 *   Unit tests for controller element mappers that contribute to a virtual axis.
 **************************************************************************************************/
...
namespace XidiTest
{
  ...
  // Creates one axis mapper for each possible virtual axis and verifies that each correctly
  // identifies its target virtual controller element.
  TEST_CASE(AxisMapper_GetTargetElement_Nominal)
  {
  ...
  // Creates and then clones one axis mapper for each possible virtual axis and verifies that each
  // correctly identifies its target virtual controller element.
  TEST_CASE(AxisMapper_GetTargetElement_Clone)
  {
    for (int i = 0; i < (int)EAxis::Count; ++i)
    {
      const AxisMapper mapperOriginal((EAxis)i);
      const std::unique_ptr<IElementMapper> mapperClone = mapperOriginal.Clone();
      TEST_ASSERT(nullptr != dynamic_cast<AxisMapper*>(mapperClone.get()));
  ...
  // Verifies the nominal behavior in which an axis mapper is asked to contribute some arbitrary
  // analog value to an axis. Sweeps the entire range of possible analog values.
  ...
  TEST_CASE(AxisMapper_ContributeFromAnalogValue_Nominal_HalfAxisPositive)
  {
    ...
    constexpr double kStepSize = (double)(kAnalogValueMax - kAnalogValueNeutral) /
        (double)(kAnalogValueMax - kAnalogValueMin);

    for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
    {
      constexpr AxisMapper mapper(kTargetAxis, EAxisDirection::Positive);
      const double analogValueDisplacement = (double)analogValue - (double)kAnalogValueMin;

      SState expectedState;
      ZeroMemory(&expectedState, sizeof(expectedState));
      expectedState[kTargetAxis] =
          kAnalogValueNeutral + (int32_t)(analogValueDisplacement * kStepSize);
  ...
  TEST_CASE(AxisMapper_ContributeFromAnalogValue_Nominal_HalfAxisNegative)
  {
    ...
    for (int32_t analogValue = kAnalogValueMin; analogValue <= kAnalogValueMax; ++analogValue)
    {
      constexpr AxisMapper mapper(kTargetAxis, EAxisDirection::Negative);
      const double analogValueDisplacement = (double)analogValue - (double)kAnalogValueMin;
  ...
  // It is possible and acceptable that the result of aggregating all contributing axis mappers
  // exceeds the maximum possible analog axis value.
  ...
  // Verifies correct behavior when multiple axis mappers all contribute to the same virtual axis
  // but the net contribution sums to the neutral position.
  TEST_CASE(AxisMapper_ContributeFromAnalogValue_DestructiveInterference)
  {
    ...
    constexpr AxisMapper mappersPositive[] = {
        AxisMapper(kTargetAxis), AxisMapper(kTargetAxis), AxisMapper(kTargetAxis)};
  ...
  // Verifies correct behavior when multiple axis mappers all contribute to the same virtual axis
  // but sourced by a button state. The aggregated contribution should be the sum of the values
  ...
  // Verifies correct behavior when multiple axis mappers all contribute to the same virtual axis
  // but sourced by a button state. In this case, the aggregate contribution sums to a net of the
  // neutral position (i.e. there are as many button states "pressed" as "not pressed").
  ...
  // Verifies the nominal behavior in which an axis mapper is asked to contribute some arbitrary
  // trigger value to an axis. Sweeps the entire range of possible trigger values.
  TEST_CASE(AxisMapper_ContributeFromTriggerValue_Nominal_EntireAxis)
  {
    constexpr EAxis kTargetAxis = EAxis::RotX;
    constexpr double kStepSize =
        (double)(kAnalogValueMax - kAnalogValueMin) / (double)(kTriggerValueMax - kTriggerValueMin);
    ...
    for (int32_t triggerValue = kTriggerValueMin; triggerValue <= kTriggerValueMax; ++triggerValue)
    {
      ...
      ZeroMemory(&expectedState, sizeof(expectedState));
      expectedState[kTargetAxis] =
          kAnalogValueMin + (int32_t)(triggerValueDisplacement * kStepSize);
  ...
  TEST_CASE(AxisMapper_ContributeFromTriggerValue_Nominal_HalfAxisPositive)
  {
    ...
    for (int32_t triggerValue = kTriggerValueMin; triggerValue <= kTriggerValueMax; ++triggerValue)
    {
      ...
      ZeroMemory(&expectedState, sizeof(expectedState));
      expectedState[kTargetAxis] =
          kAnalogValueNeutral + (int32_t)(triggerValueDisplacement * kStepSize);
  ...
  {
    constexpr EAxis kTargetAxis = EAxis::RotZ;
    constexpr double kStepSize = (double)(kAnalogValueNeutral - kAnalogValueMin) /
        (double)(kTriggerValueMax - kTriggerValueMin);
  ...
  // Verifies correct behavior when multiple axis mappers all contribute to the same virtual axis
  // but sourced by a trigger value. The aggregated contribution should be the sum of the values
  // contributed by each axis mapper, which themselves should be extreme positive based on the test
  // parameters.
  ...
  // Verifies correct behavior when multiple axis mappers all contribute to the same virtual axis
  // but sourced by a trigger value. In this case, the aggregate contribution sums to a net of the
  // neutral position.
