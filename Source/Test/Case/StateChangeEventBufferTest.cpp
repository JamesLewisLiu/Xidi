...
namespace XidiTest
{
  ...
  /// Event data used for tests. Individual test cases can use all, or just a subset, of this test
  /// input set. Actual content does not matter (it is only checked for equality) but is
  /// nevertheless spread out over several axes and buttons.
  static constexpr StateChangeEventBuffer::SEventData kTestEventData[] = {
      ...
      {.element = {.type = EElementType::Pov},
       .value = {.povDirection = {false, true, false, false}}},
      {.element = {.type = EElementType::Button, .button = EButton::B7}, .value = {.button = true}},
      {.element = {.type = EElementType::Button, .button = EButton::B2},
       .value = {.button = false}},
      {.element = {.type = EElementType::Pov},
       .value = {.povDirection = {false, false, false, false}}},
      {.element = {.type = EElementType::Axis, .axis = EAxis::Z}, .value = {.axis = 5555}},
      {.element = {.type = EElementType::Pov},
       .value = {.povDirection = {false, false, false, true}}},
      {.element = {.type = EElementType::Button, .button = EButton::B1}, .value = {.button = true}},
      {.element = {.type = EElementType::Button, .button = EButton::B1},
       .value = {.button = false}},
      ...
      {.element = {.type = EElementType::Pov},
       .value = {.povDirection = {false, false, false, false}}},
      {.element = {.type = EElementType::Button, .button = EButton::B7},
       .value = {.button = false}},
  };
  ...
  // Verifies correct behavior in the nominal case of inserting some events and then removing them
  // in order. The event buffer capacity is well above number of events being inserted, so there is
  // no issue and therefore the buffer should never report overflow. Insertion and removal is
  // repeated enough times to ensure total volume of event data exceeds the capacity, although the
  // buffer is always mostly empty. Capacity and number of repeats are chosen to be off alignment
  // with each other, but otherwise the specific values are not important.
  TEST_CASE(StateChangeEventBuffer_Nominal)
  {
    ...
    for (int i = 0; i < kTestRepeatTimes; ++i)
    {
      ...
      // Finally remove events one at a time, ensuring the count decrements each time and that the
      // event removed is actually the oldest one.
  ...
  // Verifies the buffer correctly retains its contents as the buffer size increases.
  // No overflow condition is triggered. The event buffer shrinks to exactly the size needed to hold
  // all of the events in it without overflowing.
  ...
  // Verifies correct behavior in the case of an overflow due to appending more events than the
  // buffer can hold. The most recent events should remain, and the buffer should indicate an
  // overflow condition.
  TEST_CASE(StateChangeEventBuffer_OverflowAppend)
  {
    ...
    // Check that the number of events actually maintained in the buffer is one less than its
    // capacity. This is documented buffer behavior intended for consistency with
    ...
    // Check the events themselves. The contents of the buffer should be the most-recently-appended
    // events.
  ...
  // Verifies correct behavior in the case of an overflow due to appending events and then shrinking
  // the buffer. The most recent events should remain, and the buffer should indicate an overflow
  // condition.
  ...
  // Verifies that the event buffer properly empties itself without issue if the number of events to
  // pop is in excess of the number of events present.
  ...
  // Verifies that the event buffer correctly reports is enabled and disabled status based on its
  // capacity.
