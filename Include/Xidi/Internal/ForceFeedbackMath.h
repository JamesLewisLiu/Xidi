...
namespace Xidi
{
  namespace Controller
  {
    namespace ForceFeedback
    {
      ...
      /// Helps with avoiding imprecision error when using conversions between degrees and radians,
      /// which happens internally. The value used is equal to 1/64, also known as 2e-6.
      ...
      constexpr TEffectValue AngleDegreeHundredthsToRadians(TEffectValue angle)
      {
      ...
      constexpr TEffectValue AngleRadiansToDegreeHundredths(TEffectValue angle)
      {
      ...
      /// @param [in] roundToMultiple Desired multiple of the input to which the input should be
      /// rounded.
      ...
      inline TEffectValue TrigonometryCosine(TEffectValue angle)
      {
        return NearestMultiple(
            std::cos(AngleDegreeHundredthsToRadians(angle)), kMathRoundingPrecision);
      }
      ...
      {
        return NearestMultiple(
            std::sin(AngleDegreeHundredthsToRadians(angle)), kMathRoundingPrecision);
      }
      ...
      /// @return Inverse tangent in hundredths of degrees of the numerator divided by the
      /// denominator.
      inline TEffectValue TrigonometryArcTanOfRatio(
          TEffectValue numerator, TEffectValue denominator)
      {
        const TEffectValue rawAngle = NearestMultiple(
            AngleRadiansToDegreeHundredths(std::atan2(numerator, denominator)),
            kMathRoundingPrecision);
