...
namespace Xidi
{
  namespace Controller
  {
    namespace MapperParser
    {
      ...
      /// portions. For example, the string "Axis(RotY, +)" would be separated into "Axis" as the
      /// type and "RotY, +" (the entire contents of the parentheses) as the parameters.
      struct SStringParts
      {
        /// String identifying the top-level type.
        std::wstring_view type;

        /// String holding all of the parameters without the enclosing parentheses.
        std::wstring_view params;

        /// Remaining part of the string that was not separated into parts.
        std::wstring_view remaining;

        constexpr bool operator==(const SStringParts& other) const = default;
      };

      /// Holds the result of parsing and consuming a single element mapper worth of input string.
      struct SElementMapperParseResult
      {
        /// Element mapper object, if the parse was successful. Note that `nullptr` indicates
        /// successful parse of a null element mapper.
        ElementMapperOrError maybeElementMapper;

        /// Remaining unparsed part of the string. Either empty or contains additional element
        /// mapper strings to be parsed.
        std::wstring_view remainingString;
      };

      /// Holds a partially-separated representation of a parameter string.
      struct SParamStringParts
      {
        /// String representing just the first parameter.
        std::wstring_view first;

        /// String representing all the remaining parameters.
        std::wstring_view remaining;

        constexpr bool operator==(const SParamStringParts& other) const = default;
      };

      /// Attempts to identify the index within the `all` member of #UElementMap that corresponds to
      /// the controller element identified by the input string. See "MapperParser.cpp" for strings
      /// that will be recognized as valid.
      /// @param [in] controllerElementString String to parse that supposedly identifies a
      /// controller element.
      ...
      /// Attempts to identify the index within the `all` member of #UForceFeedbackActuatorMap that
      /// corresponds to the force feedback actuator identified by the input string. See
      /// "MapperParser.cpp" for strings that will be recognized as valid.
      /// @param [in] ffActuatorString String to parse that supposedly identifies a controller
      /// element.
      /// @return Force feedback actuator map array index, if it could be identified based on the
      /// input string.
      std::optional<unsigned int> FindForceFeedbackActuatorIndex(
      ...
      /// This is the main entry point intended for use when parsing force feedback actuators from
      /// strings.
      /// @param [in] ffActuatorString Input string supposedly representing a force feedback
      /// actuator.
      /// @return Force feedback actuator descriptor object if successful, error message string
      /// otherwise.
      ForceFeedbackActuatorOrError ForceFeedbackActuatorFromString(
          std::wstring_view ffActuatorString);

      /// Determines if the specified controller element string is valid and recognized as
      /// identifying a controller element. See "MapperParser.cpp" for strings that will be
      /// recognized as valid.
      ...
      /// Determines if the specified force feedback actuator string is valid and recognized as
      /// identifying a physical force feedback actuator. See "MapperParser.cpp" for strings that
      ...
      /// @param [in] elementMapperString Input string supposedly representing an element mapper.
      /// @return Recursion depth if the recursion is balanced and therefore the depth can be
      /// determined.
      std::optional<unsigned int> ComputeRecursionDepth(std::wstring_view elementMapperString);

      /// Internal function exposed for testing.
      /// Separates the supplied element mapper string into type and parameter parts, with a
      /// possible remainder, and returns the result. Example: "Axis(X)" would be split into "Axis"
      /// and "X" as type and parameters respectively. Example: "Split( Split(Button(1), Button(2)),
      /// Split(Button(3), Button(4)) )" would be split into "Split" and "Split(Button(1),
      /// Button(2)), Split(Button(3), Button(4))" as type and parameters respectively. Example:
      /// "Split(Button(1), Button(2)), Split(Button(3), Button(4))" would be split into "Split" and
      /// "Button(1), Button(2)" as type and parameters respectively, with "Split(Button(3),
      /// Button(4))" indicated as remaining.
      /// @param [in] elementMapperString Input string supposedly representing an element mapper.
      ...
      /// Internal function exposed for testing.
      /// Separates the supplied force feedback actuator string into type and parameter parts and
      /// returns the result. Force feedback actuator strings are structured like element mapper
      /// strings, except they are not allowed to have a remainder.
      ...
      /// Internal function exposed for testing.
      /// Partially parses the supplied parameter list string by extracting the first parameter and
      /// leaving behind the rest of the string. Example: "A, B, C, D" would result in "A" as the
      /// first parameter and "B, C, D" as the remaining part of the string. Example: "RotY, +"
      /// would result in "RotY" as the first parameter and "+" as the remaining part of the string.
      /// Example: "Split(Button(1), Button(2)), Split(Button(3), Button(4))" would result in
      /// "Split(Button(1), Button(2))" as the first parameter and "Split(Button(3), Button(4))" as
      ...
      /// Parameter string should consist of a string representing an axis and optionally a second
      /// string representing an axis direction.
      ...
      /// Parameter string should consist of a comma-separated list of element mappers, up to the
      ...
      /// Parameter string should consist of a string representing a mouse axis and optionally a
      /// second string representing an axis direction.
      /// @param [in] params Parameter string.
      /// @return Pointer to the new mapper object if successful, error message string otherwise.
      ...
      /// Parameter string should consist of a string representing a POV direction (for positive
      /// "pressed" contributions) and optionally a second string representing a POV direction (for
      /// negative "pressed" contributions).
      /// @param [in] params Parameter string.
      /// @return Pointer to the new mapper object if successful, error message string otherwise.
      ...
      /// Parameter string should consist of two comma-separated strings representing element
      /// mappers.
      /// @param [in] params Parameter string.
      /// @return Pointer to the new mapper object if successful, error message string otherwise.
      ...
      /// Internal function exposed for testing.
      /// Attempts to build a force feedback actuator description object that matches the default
      /// actuator configuration. No parameters are allowed.
      /// @param [in] params Parameter string.
      /// @return Force feedback actuator description object if successful, error message string
      /// otherwise.
      ForceFeedbackActuatorOrError MakeForceFeedbackActuatorDefault(std::wstring_view params);

      /// Internal function exposed for testing.
      /// Attempts to build a force feedback actuator description object that disables its
      /// associated physical actuator. No parameters are allowed.
      /// @param [in] params Parameter string.
      /// @return Force feedback actuator description object if successful, error message string
      /// otherwise.
      ...
      /// Internal function exposed for testing.
      /// Attempts to build a force feedback actuator description object in single-axis mode using
      /// the supplied parameters. Parameter string should consist of a string representing an axis
      /// and optionally a second string representing an axis direction.
      /// @param [in] params Parameter string.
      /// @return Force feedback actuator description object if successful, error message string
      /// otherwise.
      ...
      /// Internal function exposed for testing.
      /// Attempts to build a force feedback actuator description object in magnitude projection
      /// mode using the supplied parameters. Parameter string should consist of two comma-separated
      /// strings representing axes.
      /// @param [in] params Parameter string.
      /// @return Force feedback actuator description object if successful, error message string
      /// otherwise.
      ...
      /// Internal function exposed for testing.
      /// Consumes part or all of the input string and attempts to parse it into an element mapper
      /// object.
      /// @param [in] elementMapperString Input string supposedly containing the representation of
      /// an element mapper.
      /// @return Result of the parse. Failure is indicated by the absence of an element mapper
      /// object.
      SElementMapperParseResult ParseSingleElementMapper(std::wstring_view elementMapperString);

      /// Internal function exposed for testing.
      /// Consumes all of the input string and attempts to parse it into a force feedback actuator
      /// description object.
      /// @param [in] ffActuatorString Input string supposedly containing the representation of a
