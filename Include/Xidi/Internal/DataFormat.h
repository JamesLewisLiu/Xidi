 ...
 ***********************************************************************************************//**
 * @file DataFormat.h
 *   Declaration of all functionality related to parsing and formatting controller data using
 the format specified by a DirectInput application.
 **************************************************************************************************/
...
namespace Xidi
{
  ...
  /// Centered is the special case that the POV is not pressed in any direction, and all other
  /// enumerators are named after compass directions. Per DirectInput documentation, POV value is
  /// measured as hundredths of degrees clockwise from north, where "north" semantically means
  /// pressing "up" on a d-pad.
  ...
  /// Encapsulates all functionality for writing and interpreting data formatted using an
  /// application-defined DirectInput data format for game controller data. Each instance of this
  /// class is linked to one specific controller and data format. Data format objects can be queried
  /// or used to write application data packets, but the format itself cannot be updated once the
  /// object is created. To change to a different data format, it is necessary to create a new
  /// instance. See DirectInput documentation for more information on how applications define their
  /// data format.
  class DataFormat
  {
  public:
    ...
    {
      /// Size of the application data packet, in bytes. An application is required to provide this
      /// along with its data format specification.
      TOffset packetSizeBytes;

      /// All offsets in the application's data format that correspond to POVs not present in the
      /// virtual controller. These POV areas need to be initialized to "POV neutral" when writing
      /// an application data packet.
      std::set<TOffset> povOffsetsUnused;

      /// Offsets into the application's data format for axis data. One slot exists for each
      /// possible axis, indexed by axis type enumerator.
      TOffset axisOffset[(int)Controller::EAxis::Count];

      /// Offsets into the application's data format for button data. One slot exists for each
      /// possible button, indexed by button number enumerator.
      TOffset buttonOffset[(int)Controller::EButton::Count];

      /// Offset into the application's data format for POV data. Only one slot exists because a
      /// virtual controller can only have one POV.
      TOffset povOffset;

      /// Reverse map from application data format offset to virtual controller element.
      /// Applications are allowed to identify controller elements by data format offset, so this
      /// map enables that functionality.
      std::map<TOffset, Controller::SElementIdentifier> offsetElementMap;

      inline SDataFormatSpec(TOffset packetSizeBytes)
          : packetSizeBytes(packetSizeBytes),
            povOffsetsUnused(),
            axisOffset(),
            buttonOffset(),
            povOffset(kInvalidOffsetValue),
            offsetElementMap()
      {
      ...
      inline bool operator==(const SDataFormatSpec& other) const
      {
        return (
            ...
            (povOffset == other.povOffset) && (offsetElementMap == other.offsetElementMap));
      }

      /// Associates the specified element with the specified offset into the application's data
      /// format. Does not perform any bounds-checking or error-checking. This is the responsibility
      /// of the caller.
    ...
    /// Attempts to create a data format representation from an application's DirectInput data
    /// format specification. If successful, a newly-allocated instance is returned. The pointer is
    /// owned by the caller and must be approprately freed later. Failure indicates an issue with
    /// the application format specification, which is indicated to the DirectInput application by
    /// returning `DIERR_INVALIDPARAM`.
    /// @param [in] appFormatSpec Application-provided DirectInput data format specification.
    /// @param [in] controllerCapabilities Capabilities of the virtual controller for which the data
    /// format is being specified.
    /// @return Pointer to new data format representation, or `nullptr` if there is an issue with
    /// the application format specification.
    static std::unique_ptr<DataFormat> CreateFromApplicationFormatSpec(
    ...
    /// @param [in] offset Application data format offset for which an associated virtual controller
    /// element is desired.
    ...
    /// Retrieves and returns the total number of bytes in the data format represented by this
    /// object. Does not do any error checking.
    ...
    /// Checks if the application's data format associates any virtual controller element with the
    /// specified offset.
    /// @param [in] offset Offset to check.
    /// @return `true` if the offset has a virtual controller element associated with it, `false`
    /// otherwise.
    ...
    /// Checks if the application's data format associates any offset with the specified virtual
    /// controller element.
    /// @param [in] element Virtual controller element to check.
    /// @return `true` if the virtual controller element has an offset associated with it, `false`
    /// otherwise.
    ...
    /// Formats the specified virtual controller state as an application data packet and writes it
    /// to the specified buffer. Useful for providing the application with an instantaneous snapshot
    /// of the state of a virtual controller. Failure indicates an issue with the arguments passed,
    /// which is indicated to the DirectInput application by returning `DIERR_INVALIDPARAM`.
    /// @return `true` on success, `false` on failure due to invalid arguments.
    bool WriteDataPacket(
        void* packetBuffer,
  ...
  private:
    ...
    /// Objects cannot be constructed externally. This constructor requires a complete data
    /// format specification, which will be move-assigned to this object's instance variable,
    /// and a controller capabilities object which is not owned by (and must outlive) this
    /// object.
    inline DataFormat(
        const Controller::SCapabilities controllerCapabilities, SDataFormatSpec&& dataFormatSpec)
        : controllerCapabilities(controllerCapabilities), dataFormatSpec(std::move(dataFormatSpec))
    {}
